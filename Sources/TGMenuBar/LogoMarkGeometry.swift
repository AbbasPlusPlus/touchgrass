// TGMenuBar — the TouchGrass mark's raw path data.
import AppKit
import CoreGraphics
...
/// lives here and is copied: `TGOverlay.LogoMarkGeometry` (the cursor pill's badge) and
/// `Support/icon/generate.swift` (the app icon, a standalone script). If one changes,
/// change all three.
enum LogoMarkGeometry {
    ...
    static func draw(in rect: CGRect, alpha: CGFloat = 1, fit: Fit = .disc(inset: 0.02)) {
        ...
        for blade in blades {
            ...
            ctx.setFillColor(NSColor.tgHex(UInt32(hex), alpha: alpha).cgColor)
            ctx.fillPath()
        }
        ctx.restoreGState()
    }
